...
while ($true)
{
    if ((Get-Process -Name topaz_game -ErrorAction Stop) -eq $null)
    {

        Write-Host -ForegroundColor Red "Server Is Not Running"
        <#.\topaz_game.exe -launcher -dedicated -headless -window -height 300 -width 300#>
        Start-process C:\ffxi\topaz\topaz_game
